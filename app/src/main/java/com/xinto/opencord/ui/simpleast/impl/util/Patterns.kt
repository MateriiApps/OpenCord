package com.xinto.opencord.ui.simpleast.impl.util

import java.util.regex.Pattern

val PATTERN_OTHER: Pattern = Pattern.compile("""^[\s\S]+?(?=[^0-9A-Za-z\s\u00c0-\uffff]|\n| {2,}\n|\w+:\S|${'$'})""")
val PATTERN_BOLD_TEXT: Pattern = Pattern.compile("""^\*\*([\s\S]+?)\*\*(?!\*)""")
val PATTERN_ITALIC_TEXT: Pattern = Pattern.compile("""^\b_((?:__|\\[\s\S]|[^\\_])+?)_\b|^\*(?=\S)((?:\*\*|\s+(?:[^*\s]|\*\*)|[^\s*])+?)\*(?!\*)""")
